...
export default function Dashboard() {
    ...
    modifiersClassNames["holiday"] = "bg-fuchsia-100 text-fuchsia-700 rounded-full font-bold border border-fuchsia-200";
        ...
        <div className="animate-fade-in" data-testid="dashboard">
            ...
            {(currentUser.role === "admin" || currentUser.role === "manager") && stats && (
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                    {[
                        { label: "Total Employees", value: stats.total_employees, icon: Users, color: "text-slate-600" },
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
                ...
                <div className="col-span-1 md:col-span-8 flex flex-col gap-4">
                            ...
                            <CardContent className="p-8 h-full flex flex-col sm:flex-row items-center sm:items-stretch gap-8">
                                <div className="flex-1 w-full text-center sm:text-left flex flex-col justify-center">
                                    <div className="flex items-center justify-center sm:justify-start gap-3 mb-4">
                                        ...
                                        <p className="text-lg font-semibold text-slate-800">Annual Leaves</p>
                                    </div>
                                    <p className="text-sm font-medium text-slate-500 mb-1">Available Balance</p>
                                    <div className="flex items-baseline justify-center sm:justify-start gap-2">
                                        <p className="text-5xl font-bold tracking-tight text-slate-900">{totalAvailable}</p>
                                        <p className="text-xl font-medium text-slate-400">/ {totalAlloc} days</p>
                                    </div>
                                </div>
                                <div className="flex-1 w-full flex flex-col justify-center items-center sm:items-end border-t sm:border-t-0 sm:border-l border-slate-100 pt-6 sm:pt-0 sm:pl-8">
                                    <div className="w-full max-w-xs">
                                        <div className="flex justify-between text-sm mb-2 font-medium">
                                            <span className="text-slate-500">Usage</span>
                                            <span className="text-slate-700">{totalUsed} days used</span>
                                        </div>
                    ...
                    {balance && (
                        <Card className="app-card border-amber-200 bg-amber-50/30" data-testid="lop-total">
                            <CardContent className="p-6 flex items-center justify-between">
                                <div className="flex items-center gap-4">
                                    <div className="p-3 rounded-lg bg-amber-100/50">
                                        <Clock className="w-5 h-5 text-amber-600" />
                                    </div>
                                    <div>
                                        <p className="text-lg font-semibold text-slate-800">Loss of Pay (LOP)</p>
                                        <p className="text-sm font-medium text-slate-500">Unpaid leaves taken</p>
                                    </div>
                                </div>
                                <div className="text-right">
                                    {totalLop > 0 ? (
                                        <p className="text-3xl font-bold tracking-tight text-amber-700">{totalLop} <span className="text-lg font-medium text-amber-600/60">days</span></p>
                                    ) : (
                                        <p className="text-2xl font-semibold tracking-tight text-slate-400">N/A</p>
                                    )}
                                </div>
                            </CardContent>
                        </Card>
                ...
                <div className="col-span-1 md:col-span-4 row-span-2">
                    <Card className="app-card h-full" data-testid="calendar-widget">
                        <CardHeader className="pb-2">
                            <CardTitle className="text-sm font-semibold text-slate-700">Team Calendar</CardTitle>
                        </CardHeader>
                                        ...
                                        {calendarLeaves.slice(0, 5).map((l) => {
                                            ...
                                            return (
                                                <div key={l.id} className="flex items-center gap-2 text-xs">
                                                    ...
                                                    <span className="text-slate-700 font-medium">{l.employee_name}</span>
                                                    <span className="text-slate-400">{l.start_date}</span>
                                                </div>
                                            );
                                        })}
                                    </div>
                    ...
                    <Card className="app-card" data-testid="recent-leaves">
                        <CardHeader className="pb-3">
                            <div className="flex items-center justify-between">
                                <CardTitle className="text-sm font-semibold text-slate-700">Recent Leave Requests</CardTitle>
                                <button
                                ...
                                <div className="divide-y divide-slate-100">
                                    {recentLeaves.map((leave) => {
                                        ...
                                        return (
                                            <div
                                                key={leave.id}
                                                className="modern-table-row flex items-center justify-between"
                                                ...
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        <div className="flex items-center gap-2">
                                                            <p className="text-sm font-medium text-slate-800 tracking-wide">{config.label} Leave</p>
                                                            {category && (
